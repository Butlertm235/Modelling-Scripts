...
count = 1
for (day in 1:7) { #loop through each day of the week
  group_name = c(group_name, day_lookup[day]) #vector of names
  file_location = paste0("C:/Users/UKTHM001/Documents/70091126 - SW Network Delay Tool/INRIX processing/outputs/shp/queue/2022_queue_daily_",day_lookup[day],".shp")
 ...
 if (count == 1) { #legend only included in first loop
   queue_map = queue_map + 
     ...
     tm_lines(col = "dly_ln_", lwd = 4, breaks = c(0.25,1,2,3,4,Inf), palette = colours() [c(142, 148, 90, 134, 137)], #categorised display
              group = day_lookup[day], legend.col.show = TRUE, title.col = "2022 Duration of delay", 
   ...
   queue_map = queue_map + 
     ...
     tm_lines(col = "dly_ln_", lwd = 4, breaks = c(0.25,1,2,3,4,Inf), palette = colours() [c(142, 148, 90, 134, 137)], 
              group = day_lookup[day], legend.col.show = FALSE, title.col = "2022 Duration of delay", 
...
outfile_name = paste0("Queue duration by day_2022.html")
out_rda = paste0("./outputs/rda/Queue duration by day_2022.rda") #write leaflet to r data file
