...
import mnd_import
from matrix_class import Zones, MatrixStack, Matrix, load_matrix
...
MODEL_ZONES_FILE = WORKING_FOLDER / 'ZoningTemplate_Model.xlsx'
...
SYNTH_RAIL_FILES = {time_period: INPUTS_FOLDER / f'synth_rail_{time_period}.csv' 
                   for time_period in MND_TIMES.values()}
PURPOSES_TEMPRO = INPUTS_FOLDER / 'Tempro_Data.xlsx'
...
def get_mnd_purposes(mode) -> list:
    ...
    for the given mode
    '''
    return mnd_import.MND_MODES[mode]
...
def get_mnd_zones(zones_file_location: PurePath = MND_ZONES_FILE,
    ...
    return mnd_import.get_mnd_zones(zones_file_location,
                                    zones_sheet_name)
...
def get_mnd_raw(time_period: str,
    ...
    if not force_refresh:
        try:
            return mnd_import.load_mnd(load_location = outputs_folder,
    ...
    matrix_stack = mnd_import.get_mnd_matrices(
...
def combine_mnd_synth(mnd_matrices: 'MatrixStack',
    ...
    as well as what this should do with the results
    '''
...
def road_matrix_process(time_period: str,
    ...
    bus_modelpurp *= 0.8
    bus_combined = combine_mnd_synth(
        bus_modelpurp,
    ...
    #TODO - review factor used for scaling trips in context of 
    # verification evidence
    highway_mnd_matrices *= 1.5

    car_synth = get_synth(SYNTH_CAR_FILES,
                          car_str,
                          time_period)
    combined_highway_matrices = combine_mnd_synth(
    ...
    return bus_combined, combined_highway_matrices
...
def rail_matrix_process(time_period: str,
    ...
    combined_rail_matrices = combine_mnd_synth(
        rail_modelpurposes,
    ...
    return combined_rail_matrices, car_redist_output_matrices
...
def test_imports():
    for time_period in MND_TIMES.keys():
        for mode, purposes in mnd_import.MND_MODES.items():
...
if __name__ == '__main__':
    ...
    # for time_period in ['AM']:
        load_raw = True
        refresh = True
        bus, car_lgv = road_matrix_process(time_period, 
                                            load_raw_data = load_raw,
                                            force_refresh = refresh)
        print("\n\n\n\n\n\n\n\n\n\n\n\n")
        print(f"Sums:{car_lgv.matrix_sums}")
        hgv = hgv_matrix_process(time_period, 
                                  load_raw_data = load_raw,
                                  force_refresh = refresh)
        rail, car_redist = rail_matrix_process(time_period, 
                                                load_raw_data = load_raw,
                                                force_refresh = refresh)
