...
#INPUTS
TIME_PERIODS = ["AM_peak_hr", "IP_peak_hr", "PM_peak_hr"]
USER_CLASSES = ["1", "2", "3", "4", "5", "6", "7"]
PROGRAM_DIRECTORY = os.getcwd()
FILE_DIRECTORY = f"{PROGRAM_DIRECTORY}\Test\Expanded\Synths Split"
...
for time_period in TIME_PERIODS:
    ...
    for user_class in USER_CLASSES:
        matrix = pd.DataFrame()
        matrix = pd.read_csv(f"{FILE_DIRECTORY}\synth_car_{time_period}_UC{user_class}.csv", header=None)
        ...
        matrix.to_csv(f"{PROGRAM_DIRECTORY}\TEST\Expanded\synth_car_{time_period}_UC{user_class}_Trips.csv")
    
    #Output matrix stack when all user classes in the time period have been added
    stacked_matrices.to_csv(f"{PROGRAM_DIRECTORY}\Test\Expanded\synth_car_{time_period}_Trips.csv", header=False)